...
社区版不提供官方 AI 洞察市场、智能研究、AI 分析师、官方 AI 风险评估、授权数据聚合、订阅支付和企业 SLA。需要完整能力时，可访问 [官方完整版](https://www.goldenaiquant.cn/)，或在自己的环境中接入相应服务。
...
两项功能相互独立，默认均开启，可由系统管理员在后台分别开关：

- **演示模式**：指定测试账号并显示演示提示，建议绑定只读权限或模拟交易账号。
- **新手引导**：新用户首次登录时展示步骤动画，可上一步、下一步、跳过、完成或点击步骤圆点跳转。

演示模式下，相同 AI 请求会优先使用演示账号浏览器的本地缓存，避免每次演示重复调用模型。缓存不会上传到平台服务器。
...
| Mock/CSV/自定义 Provider | 智能研究、AI 分析师和第三方联网研究编排 |
| 用户自定义风险规则和 AI 接口 | 官方 AI 风险评估、动态权重和推荐逻辑 |
| 演示模式与新手引导 | 授权数据聚合、清洗、受限数据和私有生产网关 |
